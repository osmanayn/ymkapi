...
@app.get("/html-sample")
def html_sample(url: str):
    """Sayfanın ilk 8000 karakterini döndürür — yapılış bloğunu bulmak için."""
    html = fetch_html(url)
    soup = BeautifulSoup(html, "html.parser")
    ...
    # Yapılış kelimesinin etrafındaki 3000 karakteri döndür
    lower = text.lower()
    idx = lower.find("yapılış") if "yapılış" in lower else lower.find("hazırlan")
    if idx == -1:
        idx = lower.find("tarif")
    chunk = text[max(0, idx-200):idx+3000] if idx != -1 else text[:4000]
    return {"chunk": chunk, "total_chars": len(text)}
