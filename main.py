...
def nefis_structure(url: str):
    """article.recipe-inner içindeki tüm elementleri gösterir."""
    html = fetch_html(url)
    soup = BeautifulSoup(html, "html.parser")

    article = soup.select_one("article.recipe-inner")
    if not article:
        return {"error": "article.recipe-inner bulunamadı"}
...
def nefis_full(url: str):
    """Sayfadaki tüm section ve div'leri class/id ile listeler."""
    html = fetch_html(url)
    soup = BeautifulSoup(html, "html.parser")
    ...
    cooking = re.compile(
        r'(kavur|ekle|pişir|karıştır|doğra|ilave|kızart|haşla|ısıt|servis|ocak|tencere)',
        re.IGNORECASE
    )
    ...
    for el in soup.find_all(["section", "div", "article", "p"]):
        ...
        el_id = el.get("id", "")
        text = el.get_text(strip=True)
        # Pişirme kelimesi geçen VE makul uzunlukta elementler
        if cooking.search(text) and 50 < len(text) < 2000:
            # Çok fazla çocuğu olan karmaşık containerları atla
    ...
    # En kısa (en sade) olanları üste al
    results.sort(key=lambda x: x["char_count"])
    return {"results": results[:20]}


@app.get("/nefis-api-check")
def nefis_api_check(url: str):
    """Sayfadaki JS ve data attributelerini, API çağrılarını arar."""
    html = fetch_html(url)
    soup = BeautifulSoup(html, "html.parser")

    # data- attributeli elementler
    data_attrs = []
    for el in soup.find_all(True):
        attrs = {k: v for k, v in el.attrs.items() if k.startswith("data-")}
        if attrs:
            data_attrs.append({"tag": el.name, "class": el.get("class", []), "data": attrs})
        if len(data_attrs) > 20:
            break

    # Script taglerinde API URL'si ara
    api_urls = []
    for script in soup.find_all("script"):
        text = script.string or ""
        matches = re.findall(r'(https?://[^\s"\']+(?:api|json|tarif|recipe)[^\s"\']*)', text)
        api_urls.extend(matches[:5])

    # recipe-directions veya benzeri id'li elementler
    direction_divs = []
    for el in soup.find_all(id=re.compile(r'(direction|instruction|step|yapilis|adim)', re.I)):
        direction_divs.append({"id": el.get("id"), "tag": el.name, "text": el.get_text(strip=True)[:100]})

    return {
        "data_attributes": data_attrs[:20],
        "api_urls_in_scripts": list(set(api_urls))[:10],
        "direction_ids": direction_divs,
    }
