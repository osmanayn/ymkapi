...
# Yapılış adımlarını JavaScript ile yükleyen, statik olarak çekilemeyen siteler
UNSUPPORTED_SITES = {
    "nefisyemektarifleri.com": "nefisyemektarifleri.com yapılış adımlarını JavaScript ile yüklüyor, otomatik çekilemiyor. Lütfen tarifi manuel olarak girin.",
}


def check_unsupported(url: str):
    for site, message in UNSUPPORTED_SITES.items():
        if site in url:
            raise HTTPException(
                status_code=422,
                detail=f"MANUEL_GIRIS_GEREKLI: {message}"
            )
...
def get_recipe(url: str):
    ...
    # 0. Desteklenmeyen siteleri erken kontrol et
    check_unsupported(url)
...
@app.get("/amp-lists")
def amp_lists(url: str):
    """AMP sayfasındaki tüm ul/ol listelerini gösterir."""
    amp_html = fetch_amp(url)
    if not amp_html:
        return {"error": "AMP bulunamadı"}
    soup = BeautifulSoup(amp_html, "html.parser")
    lists = []
